...
class AuthClient(BaseClient):
    ...
    async def _complete_device_authorization(self) -> AuthDTO:
        ...
        try:
            ...
            session_headers = self._get_session_headers(self._session_data, self._hash)
    ...
    async def _handle_otp_authentication(self, otp_data: Dict[str, Any]) -> AuthDTO:
    ...
    async def _perform_post_otp_login(self) -> AuthDTO:
        ...
        try:
            ...
            if "errors" in result and result["errors"]:
                ...
                raise MyVerisureAuthenticationError(f"Post-OTP login failed: {error_message}")
            ...
            if login_data and login_data.get("res") == "OK":
                ...
                raise MyVerisureAuthenticationError(f"Post-OTP login failed: {error_msg}")
                    
        except Exception as e:
            _LOGGER.error("Unexpected error during post-OTP login: %s", e)
            raise MyVerisureAuthenticationError(f"Post-OTP login failed: {e}") from e
