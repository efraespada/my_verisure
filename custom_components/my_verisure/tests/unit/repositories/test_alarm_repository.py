...
class TestAlarmRepository:
    ...
    @pytest.mark.asyncio
    async def test_get_alarm_status_success(self, alarm_repository, mock_client):
        ...
        # Act
        result = await alarm_repository.get_alarm_status(installation_id, panel, capabilities)
        
        # Assert
        assert result is not None
        mock_client.get_alarm_status.assert_called_once_with(installation_id, capabilities)
    
    @pytest.mark.asyncio
    async def test_get_alarm_status_raises_exception(self, alarm_repository, mock_client):
        """Test get alarm status raises exception."""
        # Arrange
        installation_id = "12345"
        panel = "panel1"
        capabilities = "test_capabilities"
        mock_client.get_alarm_status.side_effect = MyVerisureError("Connection failed")
        
        # Act & Assert
        with pytest.raises(MyVerisureError, match="Connection failed"):
            await alarm_repository.get_alarm_status(installation_id, panel, capabilities)
    ...
    @pytest.mark.asyncio
    async def test_arm_alarm_night_success(self, alarm_repository, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_arm_alarm_night_failure(self, alarm_repository, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_arm_alarm_away_raises_exception(self, alarm_repository, mock_client):
        """Test arm alarm away raises exception."""
        # Arrange
        installation_id = "12345"
        mock_client.arm_alarm_away.side_effect = MyVerisureError("Connection failed")
    ...
    @pytest.mark.asyncio
    async def test_arm_alarm_home_raises_exception(self, alarm_repository, mock_client):
        """Test arm alarm home raises exception."""
        # Arrange
        installation_id = "12345"
        mock_client.arm_alarm_home.side_effect = MyVerisureError("Connection failed")
    ...
    @pytest.mark.asyncio
    async def test_arm_alarm_night_raises_exception(self, alarm_repository, mock_client):
        """Test arm alarm night raises exception."""
        # Arrange
        installation_id = "12345"
        mock_client.arm_alarm_night.side_effect = MyVerisureError("Connection failed")
    ...
    @pytest.mark.asyncio
    async def test_disarm_alarm_raises_exception(self, alarm_repository, mock_client):
        """Test disarm alarm raises exception."""
        # Arrange
        installation_id = "12345"
        mock_client.disarm_alarm.side_effect = MyVerisureError("Connection failed")
