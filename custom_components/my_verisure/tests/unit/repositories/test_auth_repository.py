...
class TestAuthRepository:
    ...
    async def test_login_failure(self, auth_repository, mock_client):
        ...
        mock_client.login.side_effect = MyVerisureAuthenticationError("Invalid credentials")
    ...
    @pytest.mark.asyncio
    async def test_send_otp_raises_exception(self, auth_repository, mock_client):
        """Test OTP send raises exception."""
        # Arrange
        record_id = 1
        otp_hash = "test_hash"
        mock_client.send_otp.side_effect = MyVerisureOTPError("Failed to send OTP")
    ...
    async def test_verify_otp_success(self, auth_repository, mock_client):
        ...
        mock_client._session_data = {"lang": "es", "legals": False, "changePassword": False, "needDeviceAuthorization": False}
        
        # Act
        result = await auth_repository.verify_otp(otp_code, otp_hash, device_identifiers)
    ...
    @pytest.mark.asyncio
    async def test_verify_otp_raises_exception(self, auth_repository, mock_client):
        ...
        with pytest.raises(MyVerisureOTPError, match="Invalid OTP"):
            await auth_repository.verify_otp(otp_code, otp_hash, device_identifiers)
