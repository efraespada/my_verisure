...
from api.models.dto.installation_dto import InstallationDTO, InstallationServicesDTO
...
class InstallationRepositoryImpl(InstallationRepository):
    ...
    async def get_installations(self) -> List[Installation]:
        ...
        try:
            ...
            _LOGGER.info("Client hash token present: %s", "Yes" if self.client._hash else "No")
            _LOGGER.info("Client session data: %s", self.client._session_data)
            _LOGGER.info("Client connected: %s", "Yes" if self.client._client else "No")
    ...
    async def get_installation_services(self, installation_id: str, force_refresh: bool = False) -> InstallationServices:
        ...
        try:
            ...
            _LOGGER.info("Services data keys: %s", list(services_data.keys()) if isinstance(services_data, dict) else "Not a dict")
            ...
            try:
                ...
                _LOGGER.error("Error converting to domain model: %s", domain_error)
                raise
