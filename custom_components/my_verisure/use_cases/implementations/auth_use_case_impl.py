...
class AuthUseCaseImpl(AuthUseCase):
    ...
    async def login(self, username: str, password: str) -> AuthResult:
        ...
        try:
            ...
            # Generate device identifiers (this would typically come from a device service)
    ...
    async def verify_otp(self, otp_code: str) -> bool:
        ...
        try:
            ...
            result = await self.auth_repository.verify_otp(otp_code, otp_hash, device_identifiers)
    ...
    def select_phone(self, phone_id: int) -> bool:
        ...
        if selected_phone:
            _LOGGER.info("Phone selected: ID %d - %s", phone_id, selected_phone.phone)
            return True
        else:
