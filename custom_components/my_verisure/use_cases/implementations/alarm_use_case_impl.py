...
class AlarmUseCaseImpl(AlarmUseCase):
    ...
    def __init__(self, alarm_repository: AlarmRepository, installation_repository: InstallationRepository):
        """Initialize the use case with dependencies."""
        self.alarm_repository = alarm_repository
        self.installation_repository = installation_repository
    
    async def _get_installation_info(self, installation_id: str) -> tuple[str, str]:
        """Get panel and capabilities for an installation."""
        try:
            _LOGGER.debug("Getting installation info for %s from InstallationRepository", installation_id)
            ...
            panel = installation_data.get("panel", "PROTOCOL")  # Fallback to default
            
            # Get capabilities
            capabilities = services_data.capabilities or "default_capabilities"  # Fallback to default
            
            _LOGGER.debug("Installation info retrieved - Panel: %s, Capabilities: %s", panel, capabilities)
            return panel, capabilities
            
        except Exception as e:
            _LOGGER.warning("Failed to get installation info for %s, using defaults: %s", installation_id, e)
