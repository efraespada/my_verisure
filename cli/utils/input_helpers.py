...
def select_installation(installations: List) -> Optional[str]:
    ...
    from .display import print_header, print_error, print_success, print_installation_info
    ...
    if len(installations) == 1:
        ...
        print_success(f"Usando única instalación disponible: {installation.alias}")
    ...
    while True:
        try:
            choice = input(f"Selecciona el número de instalación (1-{len(installations)}): ").strip()
            choice_num = int(choice)
            
            if 1 <= choice_num <= len(installations):
                selected_installation = installations[choice_num - 1]
                print_success(f"Instalación seleccionada: {selected_installation.alias}")
                return selected_installation.numinst
            else:
                print_error(f"Por favor selecciona un número entre 1 y {len(installations)}")
