...
class AuthCommand(BaseCommand):
    ...
    async def _status(self) -> bool:
        ...
        if session_manager.username and not session_manager.is_authenticated:
            print_info("🔄 Intentando autenticación con credenciales guardadas...")
            try:
                success = await session_manager.ensure_authenticated(interactive=False)
                if success:
        ...
        if session_manager.is_authenticated:
            ...
            if session_manager.current_installation:
                print_info(f"🏠 Instalación actual: {session_manager.current_installation}")
            else:
