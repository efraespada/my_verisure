...
class AlarmCommand(BaseCommand):
    ...
    async def _show_status(self, installation_id: Optional[str] = None) -> bool:
        """Show alarm status."""
        ...
        try:
            ...
            print_info(f"Obteniendo estado de alarma para instalación: {installation_id}")
            
            alarm_status = await self.alarm_use_case.get_alarm_status(installation_id)
    ...
    async def _arm(self, mode: str, installation_id: Optional[str] = None, confirm: bool = True) -> bool:
        """Arm the alarm."""
        ...
        try:
            ...
            if mode.lower() not in valid_modes:
                print_error(f"Modo inválido: {mode}. Modos válidos: {', '.join(valid_modes)}")
                return False
            ...
            print_info(f"Armando alarma en modo {mode} para instalación: {installation_id}")
    ...
    async def _disarm(self, installation_id: Optional[str] = None, confirm: bool = True) -> bool:
        """Disarm the alarm."""
        ...
        try:
            ...
            print_info(f"Desarmando alarma para instalación: {installation_id}")
