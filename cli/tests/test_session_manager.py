...
class TestSessionManager:
    ...
    def setup_method(self):
        ...
        with patch('cli.utils.session_manager.SessionManager._get_session_file_path') as mock_path:
    ...
    async def test_ensure_authenticated_with_saved_credentials(self):
        ...
        with patch('cli.utils.session_manager.setup_dependencies') as mock_setup, \
             patch('cli.utils.session_manager.get_auth_use_case') as mock_get_auth, \
             patch('cli.utils.session_manager.get_installation_use_case') as mock_get_install:
            
            mock_auth_use_case = AsyncMock()
            mock_install_use_case = AsyncMock()
            mock_auth_use_case.login = AsyncMock(return_value=Mock(success=True))
            mock_get_auth.return_value = mock_auth_use_case
            mock_get_install.return_value = mock_install_use_case
            
            result = await self.session_manager.ensure_authenticated(interactive=False)
            
            assert result is True
            assert self.session_manager.is_authenticated is True
            assert self.session_manager.auth_use_case == mock_auth_use_case
            assert self.session_manager.installation_use_case == mock_install_use_case
            mock_setup.assert_called_once_with(username="test_user", password="test_password")
    ...
    async def test_ensure_authenticated_no_credentials_interactive(self):
        ...
        with patch.object(self.session_manager, '_perform_interactive_login') as mock_login:
            mock_login.return_value = True
            
            result = await self.session_manager.ensure_authenticated(interactive=True)
    ...
    async def test_ensure_authenticated_no_credentials_non_interactive(self):
        ...
        with pytest.raises(Exception, match="Authentication required but interactive mode disabled"):
    ...
    async def test_get_installations_authenticated(self):
        ...
        self.session_manager.installation_use_case.get_installations = AsyncMock(return_value=["install1", "install2"])
    ...
    async def test_logout(self):
        ...
        with patch('cli.utils.session_manager.clear_dependencies') as mock_clear:
    ...
    def test_get_session_file_path(self):
        ...
        with patch('os.path.expanduser') as mock_expanduser, \
            ...
            mock_makedirs.assert_called_once_with("/home/test/.my_verisure", mode=0o700)
    ...
    def test_session_file_permissions(self):
        ...
        assert (stat.st_mode & 0o777) & 0o002 == 0  # No write permissions for others
