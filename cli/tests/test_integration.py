...
class TestCLIIntegration:
    ...
    def test_create_parser(self):
        ...
        subparsers = [action.dest for action in parser._actions if hasattr(action, 'choices')]
    ...
    async def test_main_auth_login(self, mock_auth_command):
        ...
        assert result == 0
        mock_command_instance.execute.assert_called_once_with("login", interactive=True)
    ...
    async def test_main_info_installations(self, mock_info_command):
        ...
        assert result == 0
        mock_command_instance.execute.assert_called_once_with("installations", interactive=True)
    ...
    async def test_main_alarm_status(self, mock_alarm_command):
        ...
        sys.argv = ['my_verisure_cli.py', 'alarm', 'status', '--installation-id', '12345']
        ...
        assert result == 0
        mock_command_instance.execute.assert_called_once_with("status", installation_id="12345", interactive=True)
...
class TestCLIArgumentParsing:
    ...
    def test_info_services_parsing(self):
        ...
        args = self.parser.parse_args(['info', 'services', '--installation-id', '12345'])
        assert args.command == 'info'
    ...
    def test_info_status_parsing(self):
        ...
        args = self.parser.parse_args(['info', 'status', '--installation-id', '12345'])
        assert args.command == 'info'
        assert args.action == 'status'
        assert args.installation_id == '12345'
    ...
    def test_alarm_status_parsing(self):
        ...
        args = self.parser.parse_args(['alarm', 'status', '--installation-id', '12345'])
    ...
    def test_alarm_arm_parsing(self):
        ...
        args = self.parser.parse_args(['alarm', 'arm', '--mode', 'away', '--installation-id', '12345'])
        assert args.command == 'alarm'
    ...
    def test_alarm_disarm_parsing(self):
        ...
        args = self.parser.parse_args(['alarm', 'disarm', '--installation-id', '12345'])
        assert args.command == 'alarm'
