...
class TestBaseCommand:
    ...
    def test_get_installation_id_from_session(self):
        ...
        with patch('cli.commands.base.session_manager') as mock_session_manager:
    ...
    async def test_select_installation_if_needed_single(self):
        ...
        with patch.object(self.command, 'select_installation_if_needed') as mock_select:
...
class TestAuthCommand:
    ...
    async def test_logout(self):
        """Test _logout method."""
        with patch('cli.commands.auth.session_manager') as mock_session_manager:
    ...
    async def test_status_authenticated(self):
        ...
        with patch('cli.utils.session_manager.session_manager') as mock_session_manager:
...
class TestInfoCommand:
    ...
    async def test_execute_services(self):
        ...
        with patch.object(self.command, '_show_services') as mock_show:
            ...
            result = await self.command.execute("services", installation_id="12345")
            assert result is True
    ...
    async def test_execute_status(self):
        ...
        with patch.object(self.command, '_show_status') as mock_show:
            ...
            result = await self.command.execute("status", installation_id="12345")
            assert result is True
            mock_show.assert_called_once_with(installation_id="12345")
    ...
    async def test_show_installations_success(self):
        ...
        with patch.object(self.command, 'setup') as mock_setup:
            ...
            mock_installations = [Mock(alias="Installation 1"), Mock(alias="Installation 2")]
            self.command.installation_use_case.get_installations = AsyncMock(return_value=mock_installations)
...
class TestAlarmCommand:
    ...
    async def test_execute_arm(self):
        ...
        with patch.object(self.command, '_arm') as mock_arm:
            ...
            result = await self.command.execute("arm", mode="away", installation_id="12345")
            assert result is True
            mock_arm.assert_called_once_with(mode="away", installation_id="12345")
    ...
    async def test_execute_disarm(self):
        ...
        with patch.object(self.command, '_disarm') as mock_disarm:
            ...
            result = await self.command.execute("disarm", installation_id="12345")
            assert result is True
    ...
    async def test_show_status_success(self):
        ...
        with patch.object(self.command, 'setup') as mock_setup:
            ...
            self.command.alarm_use_case.get_alarm_status = AsyncMock(return_value=Mock())
    ...
    async def test_arm_success(self):
        ...
        with patch.object(self.command, 'setup') as mock_setup, \
             patch('cli.commands.alarm.confirm_action') as mock_confirm, \
             patch.object(self.command, 'select_installation_if_needed') as mock_select:
            ...
            result = await self.command._arm(mode="away", installation_id="12345")
            assert result is True
