...
from api.fields import RESPONSE_OK, ERROR_INVALID_CREDENTIALS, ERROR_OTP_REQUIRED, ERROR_UNAUTHORIZED
...
class AuthRepositoryImpl(AuthRepository):
    ...
    async def login(self, auth: Auth, device_identifiers: DeviceIdentifiers) -> AuthResult:
        ...
        try:
            ...
            _LOGGER.info("Client hash token after login: %s", "Yes" if self.client._hash else "No")
            _LOGGER.info("Client session data after login: %s", self.client._session_data)
            ...
            if result and self.client._hash:
                _LOGGER.info("Login successful with token: %s", self.client._hash[:50] + "..." if self.client._hash else "None")
                return AuthResult(
                    success=True,
                    ...
                    change_password=self.client._session_data.get("changePassword"),
                    need_device_authorization=self.client._session_data.get("needDeviceAuthorization")
                )
            else:
                _LOGGER.error("Login failed - result: %s, hash present: %s", result, "Yes" if self.client._hash else "No")
                return AuthResult(
                    success=False,
    ...
    async def verify_otp(self, otp_code: str, otp_hash: str, device_identifiers: DeviceIdentifiers) -> AuthResult:
        """Verify OTP code."""
        try:
    ...
    async def validate_device(self, device_identifiers: DeviceIdentifiers) -> AuthResult:
        ...
        try:
            ...
            raise MyVerisureDeviceAuthorizationError(f"Device validation failed: {e}") from e
