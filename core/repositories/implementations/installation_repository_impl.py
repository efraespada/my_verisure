...
class InstallationRepositoryImpl(InstallationRepository):
    ...
    async def get_installation_services(
        ...
        try:
            ...
            # Handle both DTO and dict responses
            if hasattr(services_data, 'dict'):
                # It's a DTO, convert to dict
                services_dict = services_data.dict()
                _LOGGER.info("Converted DTO to dict with keys: %s", list(services_dict.keys()) if isinstance(services_dict, dict) else "Not a dict")
            else:
                # It's already a dict
            ...
            try:
                # If it's already a DTO, use it directly
                if isinstance(services_data, InstallationServicesDTO):
                    services_dto = services_data
                else:
                    services_dto = InstallationServicesDTO.from_dict(services_dict)
    ...
    def _get_cache_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_info = {
                "client_cache": client_cache_info,
