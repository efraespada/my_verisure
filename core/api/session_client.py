...
class SessionClient(BaseClient):
    ...
    def _get_session_file(self) -> str:
        ...
        return os.path.join(storage_dir, f"my_verisure_session_{self.user}.json")
    
    async def save_session(self, session_data: Dict[str, Any], hash_token: Optional[str] = None, 
                          refresh_token: Optional[str] = None, device_identifiers: Optional[Dict[str, Any]] = None) -> None:
        ...
        _LOGGER.warning("Saved session includes JWT token: %s", "Yes" if hash_token else "No")
        if hash_token:
            _LOGGER.warning("Saved JWT token length: %d characters", len(hash_token))
        
        # Log tokens if available
        hash_hash = session_data.get("hash") if session_data else None
        refresh_hash = session_data.get("refreshToken") if session_data else None
    ...
    async def load_session(self) -> Optional[Dict[str, Any]]:
        ...
        if session_data is None:
            _LOGGER.debug("No session file found at %s", self._get_session_file())
            return None
        ...
        try:
            ...
            _LOGGER.warning("Loaded session includes JWT token: %s", "Yes" if session_data.get("hash") else "No")
            
            # Log tokens if available
            hash_hash = session_info.get("hash") if session_info else None
            refresh_hash = session_info.get("refreshToken") if session_info else None
    ...
    def is_session_valid(self, session_data: Dict[str, Any], hash_token: Optional[str] = None) -> bool:
        ...
        _LOGGER.warning("Session appears valid (age: %d seconds, token present: %s)", 
                     session_age, "Yes" if hash_token else "No")
        return True
    
    def get_session_dto(self, session_data: Dict[str, Any], hash_token: Optional[str] = None,
                       refresh_token: Optional[str] = None, device_identifiers: Optional[Dict[str, Any]] = None) -> SessionDTO:
    ...
    def get_device_identifiers_dto(self, device_identifiers: Dict[str, Any]) -> DeviceIdentifiersDTO:
