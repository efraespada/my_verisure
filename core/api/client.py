...
class MyVerisureClient:
    ...
    def _get_session_headers(self) -> Dict[str, str]:
        ...
        _LOGGER.warning(
            "Session header created - Hash present: %s",
            "Yes" if self._hash else "No",
        )
        if self._hash:
            _LOGGER.warning("Hash length: %d characters", len(self._hash))
            _LOGGER.warning(
                "Hash preview: %s...",
                self._hash[:20] if len(self._hash) > 20 else self._hash,
            )
        else:
        ...
        _LOGGER.warning(
            "Extension header: %s", native_headers.get("Extension")
        )
    ...
    async def _execute_device_validation_direct(
        ...
        try:
            ...
            _LOGGER.warning(
                "Headers: %s", json.dumps(headers, indent=2, default=str)
            )
            _LOGGER.warning(
                "Request Body: %s",
            ...
            _LOGGER.warning(
                "Variables: %s", json.dumps(variables, indent=2, default=str)
            )
            _LOGGER.warning("==================================")

            # Make direct request
            async with self._session.post(
                VERISURE_GRAPHQL_URL, json=request_data, headers=headers
            ) as response:
                result = await response.json()
                _LOGGER.warning("=== DEVICE VALIDATION RESPONSE ===")
                _LOGGER.warning("Status: %s", response.status)
                _LOGGER.warning(
                    "Response: %s", json.dumps(result, indent=2, default=str)
                )
                _LOGGER.warning("===================================")
                return result
    ...
    async def _execute_otp_verification_direct(
        self, otp_code: str, otp_hash: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.warning(
                "Security Header: %s",
                json.dumps(security_header, indent=2, default=str),
            )
    ...
    async def _execute_installation_services_direct(
        self, installation_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self._session.post(
                ...
                result = await response.json()
                _LOGGER.warning(
                    "Direct installation services response: %s", result
                )
                return result
    ...
    async def login(self) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "Using persistent device identifiers: %s",
                "Yes" if self._device_identifiers else "No",
            )
            native_headers = self._get_native_app_headers()
            _LOGGER.warning(
                "Using native app headers: App=%s, Extension=%s",
    ...
    async def _complete_device_authorization(self) -> bool:
        ...
        try:
            ...
            if hasattr(self._client, "transport") and hasattr(
                self._client.transport, "headers"
            ):
                ...
                if isinstance(error, dict) and "message" in error:
                    ...
                    if "auth-code" in error_msg:
                        if "10001" in error_msg:
                            _LOGGER.info(
                                "OTP authentication required (extracted from error message)"
                            )
                            ...
                            try:
                                if "{" in error_msg and "}" in error_msg:
                                    ...
                                    if "data" in error_data:
                                        return await self._handle_otp_authentication(
                                            error_data["data"]
                                        )
                            except:
                                pass
    ...
    async def send_otp(self, record_id: int, otp_hash: str) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "OTP response: %s",
                json.dumps(otp_response, indent=2, default=str),
            )
            _LOGGER.warning(
                "Full result: %s", json.dumps(result, indent=2, default=str)
            )
    ...
    async def verify_otp(self, otp_code: str) -> bool:
        ...
        _LOGGER.warning("OTP Hash: %s", otp_hash)
        _LOGGER.warning(
            "OTP Data: %s", json.dumps(self._otp_data, indent=2, default=str)
        )
        _LOGGER.warning("=====================")

        try:
            # Use the same device validation mutation but with OTP verification headers
            result = await self._execute_otp_verification_direct(
                otp_code, otp_hash
            )
            ...
            _LOGGER.warning(
                "OTP verification response: %s", validation_response
            )
            ...
            if validation_response and validation_response.get("res") == "OK":
                ...
                _LOGGER.warning(
                    "Hash Token Length: %d characters",
                    len(self._hash) if self._hash else 0,
                )
                _LOGGER.warning(
                    "Refresh Token Length: %d characters",
                    len(refresh_hash) if refresh_hash else 0,
                )
                _LOGGER.warning("=====================================")
                ...
                _LOGGER.warning(
                    "Need device authorization status after OTP verification: %s",
                    need_device_authorization,
                )
                ...
                if need_device_authorization:
                    ...
                    _LOGGER.error(
                        "This device is not authorized and will require OTP on every login"
                    )
                ...
                try:
                    ...
                    if login_success:
                        ...
                        _LOGGER.info(
                            "Device authorization status: %s",
                            (
                                "Authorized"
                                if not need_device_authorization
                                else "Not Authorized"
                            ),
                        )
                        _LOGGER.info(
                            "Updated authentication token obtained: %s",
                            self._hash[:50] + "..." if self._hash else "None",
                        )
                        _LOGGER.debug("Full updated token: %s", self._hash)
                        _LOGGER.warning(
                            "Updated session data: %s", self._session_data
                        )
                        return True
                    else:
                        _LOGGER.warning(
                            "Post-OTP login failed, but OTP verification was successful"
                        )
    ...
    async def _perform_post_otp_login(self) -> bool:
        ...
        try:
            ...
            if login_data and login_data.get("res") == "OK":
                ...
                _LOGGER.info(
                    "Device authorization required: %s",
                    login_data.get("needDeviceAuthorization", False),
                )
    ...
    async def save_session(self, file_path: str) -> None:
        ...
        if self._hash:
            _LOGGER.warning(
                "Saved JWT token length: %d characters", len(self._hash)
            )

        # Log tokens if available
        hash_hash = (
            self._session_data.get("hash") if self._session_data else None
        )
        refresh_hash = (
            self._session_data.get("refreshToken")
    ...
    async def load_session(self, file_path: str) -> bool:
        ...
        try:
            ...
            if loaded_identifiers:
                ...
                _LOGGER.warning(
                    "No device identifiers in session, will generate new ones"
                )
            ...
            if self._hash:
                _LOGGER.warning(
                    "Loaded JWT token length: %d characters", len(self._hash)
                )
    ...
    async def get_installation_services(
        ...
        try:
            _LOGGER.warning(
                "About to execute installation services query for %s",
                installation_id,
            )
            # Execute the services query
            result = await self._execute_installation_services_direct(
                installation_id
            )
            _LOGGER.warning(
                "Installation services query executed successfully"
            )
    ...
    async def get_alarm_status(
        ...
        try:
            ...
            services_data = await self.get_installation_services(
                installation_id
            )
            ...
            try:
                ...
                for service in services:
                    if service.get("request") == "EST" and service.get(
                        "active"
                    ):
                        est_service = service
                        _LOGGER.info(
                            "Found EST service with idService: %s",
                            service.get("idService"),
                        )
                        break
                ...
                if est_service:
                    ...
                    check_alarm_result = (
                        await self._execute_check_alarm_direct(
                    ...
                    if "errors" in check_alarm_result:
                        ...
                        error_msg = error.get("message", "Unknown error")
                        _LOGGER.error(
                            "Failed to get referenceId: %s", error_msg
                        )
                        return self._get_default_alarm_status()
                    ...
                    if check_alarm_data.get("res") != "OK":
                        error_msg = check_alarm_data.get(
                            "msg", "Unknown error"
                        )
                        _LOGGER.warning(
                    ...
                    if alarm_message:
                        _LOGGER.info(
                            "Received alarm message: %s", alarm_message
                        )
                    ...
                    _LOGGER.warning(
                        "EST service not found or not active, cannot get real-time status"
                    )
                    return self._get_default_alarm_status()
