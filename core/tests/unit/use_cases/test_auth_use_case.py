...
class TestAuthUseCase:
    ...
    @pytest.mark.asyncio
    async def test_login_raises_exception(self, auth_use_case, mock_auth_repository):
        """Test login raises exception."""
        # Arrange
        username = "test_user"
        password = "test_password"
        mock_auth_repository.login.side_effect = MyVerisureAuthenticationError("Connection failed")
        
        # Act & Assert
        with pytest.raises(MyVerisureAuthenticationError, match="Connection failed"):
    ...
    @pytest.mark.asyncio
    async def test_verify_otp_success(self, auth_use_case, mock_auth_repository):
    ...
    @pytest.mark.asyncio
    async def test_verify_otp_failure(self, auth_use_case, mock_auth_repository):
    ...
    @pytest.mark.asyncio
    async def test_verify_otp_raises_exception(self, auth_use_case, mock_auth_repository):
        """Test OTP verification raises exception."""
        # Arrange
        otp_code = "123456"
        mock_auth_repository.verify_otp.side_effect = MyVerisureOTPError("Invalid OTP")
